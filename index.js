...
const outSocket = process.env.OUTPUT_SOCKET_PATH
  ? require("unix-dgram").createSocket("unix_dgram")
  : null;
...
io.on("connection", (socket) => {
  ...
  const slashCommands = {
    help: {
      ...
      run: function () {
        ...
        for (command in slashCommands) {
          const args = slashCommands[command]?.args
            ?.map((arg) => `«${arg}»`)
            .join(" ");
          helpText += `/${command} ${args ? args + " " : ""}– ${
            slashCommands[command].description
          }\n`;
        }
    ...
    "map-list": {
      ...
      run: function (userId, keycode, inputType, inputName) {
        let listing =
          "Your current key-input mappings (format: key type name):\n";
  ...
  socket.on("chat message", (message) => {
    if (message.startsWith("/")) {
      ...
      const command =
        spaceIndex === -1 ? message : message.substring(0, spaceIndex);
      ...
      const args = commandInfo?.rawArg
        ? [message]
        : message.split(" ").filter((s) => s !== "");
      const expectedArgsCount = commandInfo?.args?.length ?? 0;
      if (args.length !== expectedArgsCount) {
        sendSystemMessage(
          `expected ${expectedArgsCount} arguments but got ${args.length}`
        );
        return;
      }
